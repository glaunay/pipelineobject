...
const testFunc = require("./index"); // pipeline test functions
let simpleTest = false, dualTest = false;
let treePath = null, inputPath = null;
//////////////// usage //////////////////
var usage = function () {
    let str = '\n\n  Examples:\n\n';
...
commander
    ...
    .option('-s, --simple', 'make a simple test (with a simple tree, more info with -h)', () => { simpleTest = true; })
    .option('-d, --dual', 'make a dual test (with a dual tree, more info with -h)', () => { dualTest = true; })
    .parse(process.argv);
if (!simpleTest && !dualTest)
    throw usage();
testFunc.JMsetup()
    .on('ready', (jobManager) => {
    let myPipeline;
    if (simpleTest)
        myPipeline = testFunc.simpleTest(jobManager);
    else if (dualTest)
        myPipeline = testFunc.dualTest(jobManager);
